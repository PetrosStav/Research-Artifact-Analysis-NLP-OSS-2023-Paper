...
with open('./data/raa_hybrid_dataset.json', encoding='utf-8') as fin:
    hybrid_data = json.load(fin)

diff_data = [x for x in hybrid_data if x not in [y for y in synthetic_data]]
...
name_frequencies = Counter([x['Name'] for x in diff_data if 'Name' in x]).most_common()
...
percentage_changed = len([x for x in diff_data if x['Valid']=='Yes' and x['Name']==x['Name'].lower()]) / (len([x for x in diff_data if x['Valid']=='Yes' and x['Name']!=x['Name'].lower()]) + len([x for x in diff_data if x['Valid']=='Yes' and x['Name']==x['Name'].lower()]))
print('Percentage of changed artifact names: {}'.format(percentage_changed))

# Get unique snippets
all_unique_snippets = sorted(set([x['Snippet'].replace('<m>', '').replace('</m>', '') for x in diff_data]))
...
#  - special-type instances (this is automatically balanced by the artifact count)
...
for data, unique_snippets, set_name in zip(best_split_data, best_split_snippets, ['train', 'dev', 'test']):
    ...
    with open(f'./data/raa_hybrid_dataset_diff_unique_snippets_{set_name}.txt', 'w', encoding='utf-8') as out:
    ...
    with open(f'./data/raa_hybrid_dataset_diff_aug_transformed_{set_name}.json', 'w', encoding='utf-8') as out:
